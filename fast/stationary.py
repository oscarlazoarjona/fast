...
if not sage_included:
    from misc import write_equations_code, IJ
...
def analyze_zeros(row_check, col_check, rhs_check, Ne, N_excluded_mu, states):
    r"""Find zero-valued rows or columns."""
    band_zeros = False
    zero_row = []; zero_col = []
    ...
    if band_zeros:
        ...
        if zero_row == zero_col:
            ...
            for mu in zero_row:
                ...
                rhs = rhs_check[mu-1]
                if rhs:
                    aux = 'The rhs of equation '+str(mu)
                    aux += ' is non zero, but the lhs is zero.'
                    raise ValueError(s)
                else: rhs = 0
                if i == j:
                    aux = 'The population of state '+str(states[i-1])
                    aux += 'has equation 0=0.'
                    print(aux)
                elif s == 1:
                    aux = 'The real part of the coherence between states'
                    aux += str(states[i-1])+' and '+str(states[j-1])
                    aux += 'has equation 0 = '+str(rhs)
                    print(aux)
                else:
                    aux = 'The imaginary part of the coherence between states '
                    aux += str(states[i-1])+' and '+str(states[j-1])
                    aux += 'has equation 0 = '+str(rhs)
                    print(aux)
            aux = 'ERROR: the following equations'+str(excluded_mu)
            aux += ' must be excluded from A.'
            print(aux)
            aux = 'Try again giving write_stationary the argument excluded_mu='
            aux += str(excluded_mu)+'.'
            print(aux)
...
def write_stationary(path, name, laser, omega, gamma, r, Lij,
    ...
    r"""Write Fortran code to find the stationary state."""
    t0 = time()
    Ne = len(omega[0])
    Nl = len(laser)
    ...
    long_line = """open(unit=2,file='"""+path+name
    long_line += """_params.dat',status='unknown')\n"""
    if len(long_line) >= 72:
...
#    	print* \n"""
    ...
    if excluded_mu != []:
        ...
        t_extra = write_stationary(path, name, laser, omega, gamma, r, Lij,
                                   use_symbolic_phase_transformation=True,
                                   states=states, excluded_mu=excluded_mu)
...
def run_stationary(path, name, E0, laser_frequencies, spectrum_of_laser,
                   N_delta, frequency_step=None, frequency_end=None,
                   print_steps=False, specific_deltas=None,
                   save_systems=False, clone=None, use_netcdf=True):
    r"""Run an executable for the stationary state."""
    t0 = time()
    ...
    if frequency_end is not None:
        if frequency_step is not None:
            aux = 'both frequency_end and frequency_step were specified.'
